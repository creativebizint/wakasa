...
class Common
{
    ...
    public static function asyncRecalculateOrderStock($warehouseId, $orderType, $productId, $quantity) {
        $productDetails = ProductDetails::withoutGlobalScope('current_warehouse')
                    ->where('warehouse_id', '=', $warehouseId)
                    ->where('product_id', '=', $productId)
                    ->first();

        $operation =  $operation = self::getOrderOperation($orderType, $warehouseId);
        if ($operation == 'subtract') {
            $productDetails->current_stock = $productDetails->current_stock - $quantity;
        } else {
            $productDetails->current_stock = $productDetails->current_stock + $quantity;
        }
        ...
        // Create a recalculation task to be started by schedule later
        $countProductRecalculate = Recalculate::where('product_detail_id', $productDetails->id)->count();
        if ($countProductRecalculate == 0) {
            $recalculates = new Recalculate();
            $recalculates->product_detail_id = $productDetails->id;
            $recalculates->save();
        }
    }
    
    public static function getOrderOperation($orderType, $warehouseId, $diffFromWarehouse = null) {
        $operation = 'add';
        if ($orderType == 'purchase-returns' || $orderType == 'sales' || $orderType == 'subtract' || $diffFromWarehouse == $warehouseId) {
            $operation = 'subtract';
        }
        return $operation;
    }
    
    public static function getProductStockChanges($warehouseId, $productId, $startDate, $endDate)
    {
        $productDetails = ProductDetails::withoutGlobalScope('current_warehouse')
            ->where('warehouse_id', '=', $warehouseId)
            ->where('product_id', '=', $productId)
            ->select('opening_stock','created_at','product_id')
            ->first();
        $currentStock = isset($productDetails->opening_stock) && $productDetails->opening_stock != null ? $productDetails->opening_stock : 0;
        ...
        if ($startDate && $endDate) {
            $orderItems = $orderItems->whereBetween('orders.order_date', [$startDate, $endDate]);
            $stockAdjustments->whereBetween('stock_adjustments.date', [$startDate, $endDate]);
        } else if ($startDate) {
            $orderItems = $orderItems->where('orders.order_date', '>=', $startDate);
            $stockAdjustments->where('stock_adjustments.date', '>=', $startDate);
        } else if ($endDate) {
            $orderItems = $orderItems->where('orders.order_date', '<=', $endDate);
            $stockAdjustments->where('stock_adjustments.date', '<=', $endDate);
        }

        $orderItems->select('order_items.quantity','order_items.order_id','order_items.product_id');
        $orderItems = $orderItems->get();
        $stockAdjustments->select('stock_adjustments.adjustment_type','stock_adjustments.quantity','stock_adjustments.stock_adjustment_order_id');
        $stockAdjustments = $stockAdjustments->get();

        // Calculate initial stock
        if ($startDate) {
            $initialStockOrderItems = OrderItem::join('orders', 'orders.id', '=', 'order_items.order_id')
                ->where(
                    function ($query) use ($warehouseId) {
                        $query->where('orders.warehouse_id', '=', $warehouseId)
                            ->orWhere('orders.from_warehouse_id', '=', $warehouseId);
            ...
            foreach ($initialStockOrderItems as $initialStockOrderItem) {
                $operation = self::getOrderOperation($initialStockOrderItem->order->order_type, $warehouseId, $initialStockOrderItem->order->from_warehouse_id);
                if ($operation == 'subtract') {
                    $currentStock = $currentStock - $initialStockOrderItem->quantity;
                } else {
                    $currentStock = $currentStock + $initialStockOrderItem->quantity;
                }
            }
        ...
        $productStockChanges = [];
        $productStockChangesWithBalance = [];
        $productStockChangesWithBalance[] = [
            'company_id' => $productDetails->product->company_id,
            'invoice_number' => '-',
            'order_type' => 'initial',
            'warehouse' => null,
            'from_warehouse' => null,
            'user' => null,
            'order_date' => $startDate ? $startDate : $productDetails->created_at,
            'operation' => null,
            'quantity' => 0,
        ...
        foreach ($orderItems as $orderItem) {
            $operation = self::getOrderOperation($orderItem->order->order_type, $warehouseId, $orderItem->order->from_warehouse_id);
        ...
        usort($productStockChanges, function ($currentItem, $nextItem) {
            return strtotime($currentItem['order_date']) <=> strtotime($nextItem['order_date']);
        });
        ...
        foreach ($productStockChanges as $key => $item) {
            ...
            $productStockChangesWithBalance[] = $item;
        }

        return [
            'product_details' => $productDetails,
            'calculated_stock' => $currentStock,
            'stock_changes' => $productStockChangesWithBalance
        ];
    }
    ...
    public static function storeAndUpdateOrder($order, $oldOrderId, $productDetails = [], $removedItems = [], $indirectUserId = null)
    {
        $request = request();
        $loggedUser = user();
        ...
        if ($oldOrderId != "") {
            ...
            foreach ($removedOrderItems as $removedOrderItem) {
                ...
                if ($removedItem) {
                    ...
                    //* ADDENDUM
                    $orderType = $removedItem->order->order_type;
                    $warehouseId = $removedItem->order->warehouse_id;
                    $fromWarehouseId = $removedItem->order->from_warehouse_id;
                    $userId = $removedItem->order->user_id;
                    $invoiceNumber = $removedItem->order->invoice_number;
                    $orderDate = $removedItem->order->date;
                    ...
                    if ($fromWarehouseId) {
                        $otherProductDetails = ProductDetails::withoutGlobalScope('current_warehouse')
                            ->where('warehouse_id', '=', $fromWarehouseId)
                            ->where('product_id', '=', $productId)
                            ->first();
                        $oldStock = $otherProductDetails->current_stock;
                        $fromOldStock = $productDetails->current_stock;
                    } else {
                        $fromOldStock = null;
    ...
    public static function storeAndUpdateStockAdjustmentOrder($stockAdjustmentOrder, $oldStockAdjustmentOrderId, $productDetails = [], $removedItems = [], $indirectUserId = null)
    {
        $request = request();
        $loggedUser = user();
        $productItems = $request->has('product_items') ? $request->product_items : [];
        ...
        if ($oldStockAdjustmentOrderId != "") {
            ...
            foreach ($removedOrderItems as $removedOrderItem) {
                $convertedRemovedItemId = self::getIdFromHash($removedOrderItem);
                $removedItem = StockAdjustment::find($convertedRemovedItemId);
                if ($removedItem) {
                    $adjustmentType = $removedItem->adjustment_type;
                    $warehouseId = $removedItem->warehouse_id;
                    $fromWarehouseId = $removedItem->from_warehouse_id;
                    $userId = $removedItem->user_id;
                    $invoiceNumber = $removedItem->invoice_number;
                    $stockAdjustmentDate = $removedItem->date;
                    $productId = $removedItem->product_id;
                    $productDetails = ProductDetails::withoutGlobalScope('current_warehouse')
                        ->where('warehouse_id', '=', $warehouseId)
                        ->where('product_id', '=', $productId)
                        ->first();
                    $oldStock = $productDetails->current_stock;
                    $newStock = $adjustmentType == 'subtract' ? $oldStock + $removedItem->quantity :  $oldStock - $removedItem->quantity;
                    $productDetails->current_stock = $newStock;
                    $productDetails->save();
        ...
        return self::storeAndUpdateStockAdjustmentOrderItem($stockAdjustmentOrder, $productItems, $oldStockAdjustmentOrderId);
    }

    public static function storeAndUpdateStockAdjustmentOrderItem($stockAdjustmentOrder, $productItems, $oldStockAdjustmentOrderId = "")
    {
        $adjustmentType = $stockAdjustmentOrder->adjustment_type;
        $stockAdjustmentOrderDeletable = true;
        $actionType = $oldStockAdjustmentOrderId != "" ? "edit" : "add";

        $totalQuantities = 0;
        if (count($productItems) > 0) {

            $warehouseId = $stockAdjustmentOrder->warehouse_id;
            ...
            foreach ($productItems as $productItem) {
                ...
                if ($productItem->item_id == '' || $productItem->item_id == null) {
                    ...
                    $stockHistoryQuantity = $productItem->quantity;
                    $oldStockQuantity = $productItem->stock_quantity;
                } else {
                    $productItemId = self::getIdFromHash($productItem->item_id);
                    $stockAdjustment = StockAdjustment::find($productItemId);
                    $stockHistoryQuantity = $stockAdjustment->quantity != $productItem->quantity ? $productItem->quantity : 0;
                    $oldStockQuantity = $stockAdjustment->quantity;
                }

                $stockAdjustment->company_id = $stockAdjustmentOrder->company_id;
                $stockAdjustment->warehouse_id = $stockAdjustmentOrder->warehouse_id;
                $stockAdjustment->product_id = $productItem->xid;
                $stockAdjustment->invoice_number = $stockAdjustmentOrder->invoice_number;
                $stockAdjustment->date = $stockAdjustmentOrder->date;
                $stockAdjustment->quantity = $productItem->quantity;
                $stockAdjustment->notes = $productItem->notes;
                $stockAdjustment->adjustment_type = $adjustmentType;
                $stockAdjustment->created_by = $stockAdjustmentOrder->created_by;
                $stockAdjustment->stock_adjustment_order_id = $stockAdjustmentOrder->id;

                $stockAdjustment->save();

                // Update warehouse stock for product
                // self::recalculateOrderStock($warehouseId, $productId);
                self::asyncRecalculateOrderStock($warehouseId, $adjustmentType, $productId, $stockAdjustment->quantity);

                $totalQuantities += $stockAdjustment->quantity;

                // Tracking Stock History
                if ($stockHistoryQuantity != 0) {
                    $stockHistory = new StockHistory();
                    $stockHistory->warehouse_id = $warehouseId;
                    $stockHistory->invoice_number = $stockAdjustmentOrder->invoice_number;
                    $stockHistory->date = $stockAdjustment->date;
                    $stockHistory->product_id = $stockAdjustment->product_id;
                    $stockHistory->quantity = $stockHistoryQuantity;
                    $stockHistory->old_quantity = $oldStockQuantity;
                    $stockHistory->order_type = 'stock_adjustment';
                    $stockHistory->stock_type = $adjustmentType == 'add' ? 'in' : 'out';
                    $stockHistory->action_type = "add_" . $actionType;
                    $stockHistory->created_by = $stockAdjustmentOrder->created_by;
                    $stockHistory->save();
                }
            }

            $stockAdjustmentOrder->total_items = count($productItems);
        }

        $stockAdjustmentOrder->total_quantity = $totalQuantities;
        $stockAdjustmentOrder->is_deletable = $stockAdjustmentOrderDeletable;
    ...
    public static function getTransactionNumber($type, $plus=1)
    {
        $prefixs = [
            'payment-in' => 'PAY-IN-',
            'payment-out' => 'PAY-OUT-',
            ...
            'stock-transfer-returns' => 'STK-TRANS-RET-',
            'online-orders' => 'ONLI-TRANS-',
            'product-placement-in' => 'PLC-IN-',
        ...
        $selectedPrefix = $prefixs[$type] . $currentMonth;
        
        if ($type == 'stock-adjustment-add' || $type == 'stock-adjustment-subtract') {
            $lastOrderOfTheMonth = StockAdjustmentOrder::where('invoice_number', 'like', $selectedPrefix . '%')
                ->orderBy('invoice_number', 'desc')
                ->first();
        } else if ($type == 'product-placement-in' || $type == 'product-placement-out') {
            $lastOrderOfTheMonth = Placement::where('invoice_number', 'like', $selectedPrefix . '%')
                ->orderBy('invoice_number', 'desc')
                ->first();
        } else {
        ...
        if ($lastOrderOfTheMonth) {
            $lastInvoiceNumber = explode("-", $lastOrderOfTheMonth->invoice_number);
            $lastInvoiceNumber = end($lastInvoiceNumber);
        } else {
            $lastInvoiceNumber = $currentMonth . str_pad(0, 5, "0", STR_PAD_LEFT);
        }
        
        $nextNumber = (int) $lastInvoiceNumber + $plus;
        $result = $prefixs[$type] . $nextNumber;
        ...
        if ($exist) {
            return self::getTransactionNumber($type, $plus + 1);
        }
    ...
    public static function assignCompanyForNonSaas($company)
    {
        ...
        DB::table('parts')->update(['company_id' => $company->id]);
