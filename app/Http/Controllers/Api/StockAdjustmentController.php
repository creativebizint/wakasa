...
class StockAdjustmentController extends ApiBaseController
{
	...
	public function destroyed(StockAdjustment $stockAdjustment)
	{
        ...
        public function getStockAdjustmentOrderItemData()
    {
        $request = request();
        $orderIds = [];

        foreach ($request->stock_adjustment_order_id as $key => $val) {
            $orderIds[] = Common::getIdFromHash($val);
        }

        $orderItems = StockAdjustment::whereIn('stock_adjustment_order_id', $orderIds)
            ->join('stock_adjustment_orders','stock_adjustment_orders.id','stock_adjustments.stock_adjustment_order_id')
            ->join('products', 'products.id', 'stock_adjustments.product_id')
            ->join('units', 'products.unit_id', 'units.id')
            ->join('categories','categories.id','products.category_id')
            ->join('brands','brands.id','products.brand_id')
            ->select('products.item_code', 'products.image as image_url', 'products.name', 'brands.name as brand_name', 'categories.name as category_name', 'units.short_name as unit_short_name', 'stock_adjustments.*')
            ->get();
        
        $data =[];

        foreach ($orderItems as $item) {
            $hashedOrderId = Common::getHashFromId($item->stock_adjustment_order_id);
            if (!isset($data[$hashedOrderId]) || count($data[$hashedOrderId]) < 50) {
                $data[$hashedOrderId][] = $item;
            }
        }
