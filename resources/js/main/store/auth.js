...
const ALL_FRONT_STORES = 'all_front_stores';
...
const TW_PROGRESS = 'tw_progress';
const ORDERS_PROGRESS = 'orders_progress';
...
export default {
    ...
    state() {
        return {
            ...
            store: getJSONFromLocalStorage(SELECTED_STORE),
            all_stores: getJSONFromLocalStorage(ALL_STORES) || [],
            all_front_stores: getJSONFromLocalStorage(ALL_FRONT_STORES) || [],
            ...
            importProgress: getJSONFromLocalStorage(IMPORT_PROGRESS) || {
                percent: 0,
                status: "active",
                message: "",
                loading: false,
            },
            twProgress: getJSONFromLocalStorage(TW_PROGRESS) || {
                percent: 0,
                status: "active",
                message: "",
                loading: false,
            },
            ordersProgress: getJSONFromLocalStorage(ORDERS_PROGRESS) || {
                percent: 0,
                status: "active",
                message: "",
                loading: false,
    ...
    mutations: {
        ...
        updateStore(state, store) {
            state.store = store;
            window.localStorage.setItem(SELECTED_STORE, JSON.stringify(store));
        },
        updateAllStores(state, stores) {
            state.all_stores = stores;
            window.localStorage.setItem(ALL_STORES, JSON.stringify(stores));
        },
        ...
        updateImportProgress(state, importProgress) {
            state.importProgress = importProgress;
            window.localStorage.setItem(IMPORT_PROGRESS, JSON.stringify(importProgress));
        },
        updateTwProgress(state, twProgress) {
            state.twProgress = twProgress;
            window.localStorage.setItem(TW_PROGRESS, JSON.stringify(twProgress));
        },
        updateOrdersProgress(state, ordersProgress) {
            state.ordersProgress = ordersProgress;
            window.localStorage.setItem(ORDERS_PROGRESS, JSON.stringify(ordersProgress));
        },
    },
    ...
    actions: {
        ...
        updateAllStores(context) {
            axiosAdmin.get('/stores?limit=10000')
                .then(function (response) {
                    context.commit('updateAllStores', response.data);
                })
                .catch(function (error) {
