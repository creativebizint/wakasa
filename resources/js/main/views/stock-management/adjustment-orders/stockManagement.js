...
const stockManagement = () => {
    ...
    const fetchAllSearchedProduct = (value) => {
        ...
        if (value != "") {
            console.log(orderPageObject.value);
    ...
    const recalculateValues = (product) => {
        ...
        var unitPrice = (unitPrice = parseFloat(product.single_unit_price));
        var discountRate = product.discount_rate;
        if (product.price_type === "retail_counter_price") {
            unitPrice = product.retail_counter_price;
            discountRate =
                product.discount_counter_price !== "SP"
                    ? product.discount_counter_price
                    : 0;
        } else if (product.price_type === "retail_online_price") {
            unitPrice = product.retail_online_price;
            discountRate =
                product.discount_online_price !== "SP"
                    ? product.discount_online_price
                    : 0;
        } else if (product.price_type === "special_counter_price") {
            unitPrice = product.special_counter_price;
            discountRate = 0;
        } else if (product.price_type === "special_online_price") {
            unitPrice = product.special_online_price;
            discountRate = 0;
        } else if (product.price_type === "purchase_price") {
    ...
    //* ADDENDUM
    const subtotalChanged = (record) => {
        record.discount_rate = parseFloat(
            (
                100 -
                (record.subtotal * 100) /
