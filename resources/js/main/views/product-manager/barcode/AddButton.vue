<template>
	<div v-if="permsArray.includes('barcode_create') || permsArray.includes('admin')">
		<template v-if="customType == 'menu'">
			<a-menu-item @click="showAdd" key="barcode">
				<template #icon>
					<slot name="icon"></slot>
				</template>
				<slot></slot>
		...
		<AddEdit
			...
			:formData="formData"
			:data="formData"
			:pageTitle="$t('barcode.add')"
			:successMessage="$t('barcode.created')"
		/>
	</div>
</template>
<script>
